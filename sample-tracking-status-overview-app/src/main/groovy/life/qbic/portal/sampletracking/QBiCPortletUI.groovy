...
abstract class QBiCPortletUI extends UI {
    ...
    static final String DEFAULT_VERSION = "0.2.0-SNAPSHOT"
    ...
    @Override
    final void setContent(final Component content) {
        super.setContent(content)
    }

    // adds version and repository information to the passed layout
    private void addPortletInfo(final Layout layout) {
        final Label portletInfoLabel = new Label("version " + portletVersion +
                " (<a href=\"" + portletRepoURL + "\">" + portletRepoURL + "</a>)",
                ContentMode.HTML)
